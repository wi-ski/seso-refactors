...
import { FormatCodeSettings, IndentationText, LanguageService, Node, Project,SyntaxKind,UserPreferences } from "ts-morph";
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const pluckMandatorVsCodeAttrsOrThrow = (vscodeLibRef: typeof vscode)=>{
    ...
    if(!workspaceFolders){throw new Error("Bad workspaceFolders");};
    if(!activeTextEditor){throw new Error("Bad activeTextEditor");};
  ...
  const removeFileFromProject=(e: any)=>{
    ...
    if(sf) {return taskQueue.push(async () => project.removeSourceFile(sf));}
    console.error(`(Ignoring file system event REMOVE) Attempted to remove untrack SourceFile: ${e.path}`);
  };
  const addFileToProject=(e: any)=>{
    console.log("project.getSourceFileOrThrow(e.path).addSourceFileAtPath()");
    console.log(e.path);
    const sf = project.getSourceFile(e.path);
    if(sf) {return taskQueue.push(async () => project.addSourceFileAtPathIfExists(e.path));}
    console.error(`(Ignoring file system event ADD) Attempted to add untrack SourceFile: ${e.path}`);
  };
  const updateFileInProject=(e: any)=>{
    console.log("project.getSourceFileOrThrow(e.path).refreshFromFileSystem()");
    console.log(e.path);
    const sf = project.getSourceFile(e.path);
    if(sf) {return taskQueue.push(async () => project.getSourceFileOrThrow(e.path).refreshFromFileSystem());}
    console.error(`(Ignoring file system event UPDATE) Attempted to update untrack SourceFile: ${e.path}`);
  };
  ...
  const disposable = vscode.commands.registerCommand('seso-refactors.helloWorld', async () => {
    console.time();
    try {
      ...
      const _destinationDomain = await vscode.window.showInputBox({
        ...
        validateInput: _v => {
          ...
          const endsRight = v.endsWith("Domain.ValueObject")|| v.endsWith("Domain.Entity")|| v.endsWith("Application.DTO")|| v.endsWith("Infrastructure.Schema");
          if(!startsWithTSesoPrefix){return  "Must start with TSeso.TD.";};
          if(!isNotTooDepp) {return "Path too deep.";};
          if(!endsRight) {return "Invalid path.";};
          const array = v.split(".");
          for (let index = 0; index < array.length; index++) {
            const element = array[index];
            const firstChar = String(element.split("")[0]);
            if (firstChar.toUpperCase() === firstChar) {continue;};
      ...
      const destinationDomain = _destinationDomain.split(tsesoPrefix).pop()?.split(".").filter(s => s!==".").join(".");
      if(!destinationDomain) {throw new Error("Bad destinationDomain");};
  
      const domainFolderPart =  "ddd";
      const defaultDomainBarrelExportFileContent = () => `export * as Application from "./application/types";
  export * as Domain from "./domain/types";
  export * as Infrastructure from "./infrastructure/types";
  `;
      const defaultInfrastructureTypeFileContent = () => `export * as Schema from "./Schema";`;
      const defaultApplicationTypeFileContent = () => `export * as DTO from "./DTO";`;
      const defaultDomainTypeFileContent = () => `export * as Entity from "./Entity";
  export * as ValueObject from "./ValueObject";
      `;
      const emptyTypeFileContent = () => `import type * as TSeso from "@/lib/types";
  export {};
      `;
      const topLevelNamespace= "TSeso.TD";
      const domainFilePathPart = (() => {
        if (destinationDomain.includes(".Application.")) {
          return `${destinationDomain.split(".Application.")[0].split(".").join("/")}`;
        }
        if (destinationDomain.includes(".Domain.")) {
          return `${destinationDomain.split(".Domain.")[0].split(".").join("/")}`;
        }
        if (destinationDomain.includes(".Infrastructure.")) {
          return `${destinationDomain.split(".Infrastructure.")[0].split(".").join("/")}`;
        }
      })() as string;
      const defaultArgs = {
        destinationDomain,
        ...
        if (destinationDomain.includes(".Application.")) {
          ...
          return `${path.join(pwd,domainFolderPart,domainFilePathPart,secondPart,"types",thirdPart)}.ts`;
        }
        if (destinationDomain.includes(".Domain.")) {
      ...
      const formatFilePretty = (sourceFile: SourceFile, formSettings: FormatCodeSettings, uprefs: UserPreferences) => {
      ...
      const getTypesSingleDefintionNodeAndSourceFileOrThrow = (languageService: LanguageService ,n: Node): { node: Node, sourceFile: SourceFile, symbolNameAtDefintion: string } => {
        console.log(`DOING - getTypesSingleDefintionNodeAndSourceFileOrThrow for: ${n.getText()}. File: ${n.getSourceFile().getFilePath()}`);
        console.timeLog();
        
        const defintions = languageService.getDefinitions(n).map(n => ({ node:n.getNode(), sourceFile: n.getSourceFile(), symbolNameAtDefintion: n.getNode().getSymbolOrThrow().getName() }));
        if(defintions.length > 1) {throw new Error(`Found more than one defintion, files: ${defintions.map(d => d.sourceFile.getSourceFile().getFilePath()).join(",")}`);};
        const d = defintions.pop();
        if(!d) {throw new Error("Didn't find a defintion node");};
        const symbolNameAtDefintion = d.symbolNameAtDefintion;
        console.log(`DONE - getTypesSingleDefintionNodeAndSourceFileOrThrow for: ${n.getText()}. File: ${n.getSourceFile().getFilePath()}`);
        console.log(`symbolNameAtDefintion: ${symbolNameAtDefintion}`);
        console.timeLog();
        return d;
      };
      
      const getDescendantAtPosOrThrow = (sourceFile: SourceFile, pos: number) => {
        const n = sourceFile.getDescendantAtPos(pos); 
        if(n) {return n;};
        throw new Error(`Could not find desc in file ${sourceFile.getFilePath()} at position ${pos}`);
      };
      function buildPathsToFileContent(object: Record<string, any>): { content: string; path: string }[] {
        const result: any[] = [];
      ...
      const createTypeFilesIfNotExistOrAppendToExisting = (project: Project, domainShapeConf: (typeof defaultArgs)["domainShapeConfig"]) => {
        const pathsToContent = buildPathsToFileContent(domainShapeConf).map(o => {
          return {...o, path: `${pwd}/${defaultArgs.domainFolderPart}/${domainFilePathPart}/${o.path}`};
        });
        pathsToContent.forEach((pathToContentObj) => {
          console.log(`Attempting to instantiate file: ${pathToContentObj.path}`);
          console.timeLog();
          try {
            project.createSourceFile(pathToContentObj.path, pathToContentObj.content, {
              overwrite: false,
            });
      ...
      console.log("DOING - project.manipulationSettings.getFormatCodeSettings();");
      console.timeLog();
      const formatCodeSettings = project.manipulationSettings.getFormatCodeSettings();
      console.log("DOING - project.manipulationSettings.getUserPreferences();");
      console.timeLog();
      ...
      if(!activeTextEditor) {throw new Error("No active editor");};
      ...
      const onlyDefintionNodeForSelectedNode = getTypesSingleDefintionNodeAndSourceFileOrThrow(languageService, targetNode);
      console.log({
        onlyDefintionNodeForSelectedNode: onlyDefintionNodeForSelectedNode.node.getText(),
        symbolNameAtDefintion: onlyDefintionNodeForSelectedNode.symbolNameAtDefintion,
        getFilePath: onlyDefintionNodeForSelectedNode.sourceFile.getFilePath(),
      });
      const args = {
        ...defaultArgs,
        ...
        targetRefactoringSourceFile: onlyDefintionNodeForSelectedNode.sourceFile,
        definitionName: onlyDefintionNodeForSelectedNode.node.getSymbolOrThrow().getName(),
      ...
      createTypeFilesIfNotExistOrAppendToExisting(project, args.domainShapeConfig);
  
      const initialNodeAtCursorPositionToFindDeclarationOf = sourceFile.getDescendantAtPos(offset);
      if(!initialNodeAtCursorPositionToFindDeclarationOf) {throw new Error("Bad initialNodeAtCursorPositionToFindDeclarationOf");};
      const defintions = languageService.getDefinitions(initialNodeAtCursorPositionToFindDeclarationOf);
      const declarationNodeForDefintion = defintions[0].getDeclarationNode();
      // Only handling if has single defintion currently.
      if(defintions.length > 1) {throw new Error("Bad declarationNodeForDefintion length");};
      if(!declarationNodeForDefintion) {throw new Error("Bad declarationNodeForDefintion");};
  
  
      const targetRefactorSourceFile = declarationNodeForDefintion.getSourceFile();
      const destinationRefactorSourceFile = project.getSourceFileOrThrow(defaultArgs.destinationFilePath);
  
      console.log("Crawling the tree...");
      console.timeLog();
      const referencesToDeclarationNodeToIterateAndMaybeReplace = languageService.findReferences(declarationNodeForDefintion);
      referencesToDeclarationNodeToIterateAndMaybeReplace.forEach((declarationNodeReference) => {
        declarationNodeReference.getReferences().forEach((dd) => {
          ...
          if(parentKind === "TypeAliasDeclaration") {
            ...
            destinationRefactorSourceFile.addTypeAlias({
              isExported: true,
              ...parentNode.asKindOrThrow(SyntaxKind.TypeAliasDeclaration).getStructure(),
            });
          };
          ...
          if(["TypeReference", "QualifiedName"].includes(parentKind)) {
            ...
            parentNode.replaceWithText(`TSeso.TD.${args.destinationDomain}.${args.definitionName}`);
            return;
          };
      ...
      console.timeLog();
      console.log({
        declarationNodeForDefintiongetText:declarationNodeForDefintion.getText(),
        declarationNodeForDefintiongetFilePath:declarationNodeForDefintion.getSourceFile().getFilePath(),
        declarationNodeForDefintiongetFileText:declarationNodeForDefintion.getSourceFile().getText()
      });
      declarationNodeForDefintion.asKindOrThrow(SyntaxKind.TypeAliasDeclaration).remove();
      ...
      console.timeLog();
      args.trackedFiles.forEach(f => formatFilePretty(f,formatCodeSettings, userPreferences));
      ...
      await vscode.window.showInformationMessage(`New Location: ${destinationRefactorSourceFile.getFilePath()}`);
    } catch (error) {
